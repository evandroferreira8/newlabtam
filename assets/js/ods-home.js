(()=>{
  ...
  function drawCards(){
    grid.innerHTML=ods.map((item,index)=>`<button class="ods-card${index===0?' active':''}" type="button" data-ods="${item.id}" aria-pressed="${index===0?'true':'false'}"><span class="ods-card-number">${item.id}</span><strong>${esc(item.title)}</strong></button>`).join('');
